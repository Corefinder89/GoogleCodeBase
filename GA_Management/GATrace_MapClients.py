...
def get_client_list(config):
    ...
    response = requests.get ('https://www.googleapis.com/analytics/v3/management/accounts', headers=headers)

    response_data = response.json()

    csv = open("GA1.csv", "w")
    columnTitleRow = "AccountName, AccountID\n"
    csv.write(columnTitleRow)
    csv.close()

    for i in response_data['items']:
        acct_dict = {i['name']:i['id']}
